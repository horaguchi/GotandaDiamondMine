...
GotandaDiamondMine.STATE_DEFEATED     = 6;
GotandaDiamondMine.STATE_VICTORY      = 7;
...
GotandaDiamondMine.prototype.pointConfirm = function (x, y) {
  if (0 <= x && x <= 12 && 45 <= y && y <= 47) { // Undo
    //this.selectedPlace = this.items[this.items.length - 1][2];
...
GotandaDiamondMine.prototype.pointAnimation = function (x, y) {
  ...
  if (!this.waveWait) {
    if (this.path.length - 1 == this.waveState) { // animation end
      ...
      ++this.wave;
      if (this.status.hp <= 0) {
        this.state = GotandaDiamondMine.STATE_DEFEATED;
      } else if (this.wave == this.waves.length) {
        this.state = GotandaDiamondMine.STATE_VICTORY;
      } else {
...
GotandaDiamondMine.prototype.createMap = function () {
  ...
  var points_num = Math.floor(Math.random() * 4) + 2;
  var points = [ ];
...
GotandaDiamondMine.prototype.getScreen = function () {
  ...
  if (state == GotandaDiamondMine.STATE_TITLE) {
    ...
    return this.getScreenToDefeated();
  } else if (state == GotandaDiamondMine.STATE_VICTORY) {
    return this.getScreenToVictory();
  }
};
...
GotandaDiamondMine.prototype.getScreenToDefeated = function () {
  return [].concat(this.getWaveInfo(), this.getLog(), this.getMap(), this.getStatus(), this.getItemInfo(), this.getButton());
};

GotandaDiamondMine.prototype.getScreenToVictory = function () {
  return [].concat(this.getWaveInfo(), this.getLog(), this.getMap(), this.getStatus(), this.getItemInfo(), this.getButton());
};
